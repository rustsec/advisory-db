...
package = "tiberius"
date = "2026-07-29"
url = "https://github.com/tiberius-rs/tiberius/issues/424"
references = [
    "https://github.com/tiberius-rs/tiberius/issues/424",
    "https://github.com/tiberius-rs/tiberius/issues/425",
    "https://github.com/tiberius-rs/tiberius/pull/443",
]
categories = ["denial-of-service"]
keywords = ["tds", "mssql", "sql-server", "parser", "panic", "dos"]
cvss = "CVSS:3.1/AV:N/AC:L/PR:N/UI:N/S:U/C:N/I:N/A:H"
# aliases = []   # a GHSA was filed privately on 2026-07-29 and is still unpublished,
#                # so there is no resolvable alias or CVE yet
...
years and is affected by all eleven sites.

Upstream status as of 2026-09-17. The repository moved out of the Prisma organization to
`tiberius-rs` in September 2026 and took on a new maintainer team. Pull requests #440 and
#441 have since merged to the development branch and close seven of the eleven sites. Issues
#424 and #425 were closed as fixed by #440. Seven of them are. Four are not, and are still
present on the development branch, verified by running the same harness against `feb8df2` in
a release build:

- a ROW token with no preceding COLMETADATA (`token_row.rs`, `decode`)
- the same via NBCROW (`token_row.rs`, `decode_nbc`)
- a COLMETADATA column typed `SSVariant` or `Udt` (`type_info.rs`)
- an ENVCHANGE `BeginTransaction` or `EnlistDTCTransaction` whose length byte is not 8
  (`token_env_change.rs`)

Patches for those four sit in pull request #443, which is unmerged and sequenced behind a
breaking change in #442.

No release has been cut from the development branch, so `patched` above is empty and every
published version is affected by all eleven sites. Should a release ship the seven fixes
without the remaining four, this advisory will need splitting, since the two groups will
then have different patched versions.

A private advisory was filed through the repository's private vulnerability reporting on
2026-07-29 and remains unpublished, which is why no resolvable GHSA or CVE alias is listed
above.
